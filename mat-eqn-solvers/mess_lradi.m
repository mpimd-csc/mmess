...
if not(isfield(opts.shifts,'info'))
    ...
    if not(isnumeric(opts.shifts.info))&&not(islogical(opts.shifts.info))
        error('MESS:info','opts.shifts.info parameter must be logical or numeric.');
    end
end
...
if not(isfield(opts.adi,'rel_diff_tol'))||not(isnumeric(opts.adi.rel_diff_tol))
    warning('MESS:control_data',...
...
if opts.LDL_T
    ...
    if isfield(eqn, 'S_diag')
        diagonalized_RHS = 0;
    elseif isdiag(eqn.S) %%% enq.S can be a vector from lrnm and then U is unknown
  ...
  if not((isfield(opts.shifts,'p'))&&isnumeric(opts.shifts.p)&& ...
    ...
    while i<=length(opts.shifts.p)
      if not((isreal(opts.shifts.p(i))))
        if (opts.shifts.p(i+1)~=conj(opts.shifts.p(i))), illelgal_shifts=1;end
        i=i+1;
      end
      i=i+1;
    end
...
if diagonalized_RHS 
    [W, res0, eqn, opts, oper] = oper.init_res(eqn, opts, oper, eqn.G*eqn.U_diag);
else
    [W, res0, eqn, opts, oper] = oper.init_res(eqn, opts, oper, eqn.G);
end
...
while i<opts.adi.maxiter+1
    ...
    if isreal(pc)
        ...
        [ out, eqn, opts, oper ]=mess_accumulateK(eqn, opts, oper, out, pc, V);
    else
        ...
        if opts.adi.compute_sol_fac
            if opts.LDL_T
                Z(:,(i-1)*k+1:(i+1)*k) = [(sqrt(2) / a) * V1, (sqrt(2) / a) * V2];
                out.D(i : i + 1, i : i + 1) = -2 * real(pc) * eye(2);
            else
                Z(:,(i-1)*k+1:(i+1)*k) = [V1, V2];
            end
        end
        [ out, eqn, opts, oper ]=mess_accumulateK(eqn, opts, oper, out, pc, V1, V2);
