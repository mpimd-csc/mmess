function X = sol_E_default_iter(eqn, opts, opE, B, opB)
...
if not(isfield(opts.usfs.default_iter, 'X0_E'))
    opts.usfs.default_iter.X0_E = [];
end

%% Check data in eqn structure

switch lower(opts.usfs.default_iter.method_E)
    case 'pcg'
        if not(opts.usfs.default_iter.E_is_spd)
...
switch lower(opts.usfs.default_iter.method_E)
    ...
    otherwise
        mul_E = @(X) mul_E_default_iter(eqn, opts, opE, X, 'N');

end

% For calling the actual iterative solver
solver = eval(sprintf('@%s', lower(opts.usfs.default_iter.method_E)));
...
if any(flags)
    mess_warn(opts, 'usfs_iter', ...
              [lower(opts.usfs.default_iter.method_E) ...
...
Y = mul_E_default_iter(eqn, opts, my_opE, X, 'N');
end
...
Y = opts.usfs.default_iter.PE_R \ (opts.usfs.default_iter.PE_L \ X);
end
