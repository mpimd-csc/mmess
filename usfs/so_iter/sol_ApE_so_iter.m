function X = sol_ApE_so_iter(eqn, opts, opA, p, opE, C, opC)
...
if not(isfield(opts.usfs.so_iter, 'X0_ApE'))
    opts.usfs.so_iter.X0_ApE = [];
end
%% Check data in eqn structure
switch lower(opts.usfs.so_iter.method_ApE)
    case {'minres', 'pcg', 'symmlq', 'pcr'}
        mess_err(opts, 'error_arguments', ['Resulting matrix of ApE is not' ...
...
switch lower(opts.usfs.so_iter.method_ApE)
    ...
    case 'pcg'
        ...
        mul_ApE = @(X) -mul_ApE_so_iter(eqn, opts, opA, p, opE, X, 'N');
    otherwise
...
solver = eval(sprintf('@%s', lower(opts.usfs.so_iter.method_ApE)));
...
if any(flags)
    mess_warn(opts, 'usfs_iter', ...
              [lower(opts.usfs.so_iter.method_ApE) ...
...
switch lower(flag)
    case 'notransp'
        Y = mul_ApE_so_iter(eqn, opts, opA, p, opE, X, 'N');
    case 'transp'
        if strcmp(opA, 'N')
            my_opA = 'T';
        else
...
Y = opts.usfs.so_iter.PApE_R \ (opts.usfs.so_iter.PApE_L \ X);
end
