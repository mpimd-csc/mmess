function X = sol_A_so_iter(eqn, opts, opA, B, opB)
% function X=sol_A_so_iter(eqn, opts,opA,B,opB)
%
...
if not(isfield(opts.usfs.so_iter, 'X0_A'))
    opts.usfs.so_iter.X0_A = [];
end

%% Check data in eqn structure
switch lower(opts.usfs.so_iter.method_A)
    case {'minres', 'pcg', 'symmlq', 'pcr'}
        mess_err(opts, 'error_arguments', ...
                 'field eqn.A_ is not symmetric and positive definite.');
end
...
switch lower(opts.usfs.so_iter.method_A)
    ...
    case 'pcg'
        ...
        mul_A = @(X) -mul_A_so_iter(eqn, opts, opA, X, 'N');
    otherwise
...
solver = eval(sprintf('@%s', lower(opts.usfs.so_iter.method_A)));
...
if any(flags)
    mess_warn(opts, 'usfs_iter', ...
              [lower(opts.usfs.so_iter.method_A) ...
...
Y = mul_A_so_iter(eqn, opts, my_opA, X, 'N');
end
...
Y = opts.usfs.so_iter.PA_R \ (opts.usfs.so_iter.PA_L \ X);
end
